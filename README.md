...
Yakunin-Router создает платежную ссылку, показывает QR-код СБП в терминале и после успешной оплаты сохраняет локальный ключ, совместимый с OpenRouter, в `~/.iola/secrets.json`. Модели выбираются из того же каталога OpenRouter, но пользователь оплачивает пополнение российской картой/СБП. В меню API есть отдельный пункт `Пополнить баланс Yakunin-Router`.
