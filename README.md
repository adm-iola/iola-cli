...
Для обычной установки на Windows используйте premium `.exe`-установщик из раздела [Releases](https://github.com/adm-iola/iola-cli/releases).
...
- создает отдельный локальный профиль данных через `IOLA_HOME` внутри выбранной папки установки;
- создает ярлык в меню Пуск и, по выбору пользователя, на рабочем столе;
- проверяет Node.js и при необходимости пытается установить его через `winget`;
- предлагает сразу запустить мастер настройки.
