...
- `domru-intercom` - когда запрос связан с домофоном Дом.ру: адреса, домофоны, открыть дверь, история событий, временные коды;
...
Toolset `domru` включает tools для личного домофона Дом.ру:

- `domru_status` - проверить подключение;
- `domru_places` - адреса пользователя;
- `domru_intercoms` - список домофонов;
- `domru_open_intercom` - открыть домофон по ID;
- `domru_call_history` - история событий;
- `domru_temporal_codes` - временные коды.

Открытие домофона требует явного подтверждения `confirm=true`. Логин, пароль, access token и refresh token хранятся локально и не выводятся в ответах.
